<template>
  <div>
    ...
    <div class="mb-6 flex justify-between items-center">
      <search-filter v-model="form.search" class="w-full max-w-md mr-4" @reset="filterChange('reset')" @input="filterChange">
        <label class="block text-gray-700">Trashed:</label>
        <select v-model="form.trashed" class="mt-1 w-full form-select" @change="filterChange($event.target.value)">
          <option :value="null" />
...
export default {
  ...
  methods: {
    filterChange: throttle(function(value) {
      ...
      this.$inertia.get(route(route().current()), query, { replace: true, preserveState: true })
    }, 500),
  },
}
</script>
